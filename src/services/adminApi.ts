...
export async function getAnalysisList(page = 1, limit = 20, search = '', status = ''): Promise<AnalysisListResponse> {
    try {
        ...
        const response = await apiRequest<any[]>(`/api/analysis?${queryParams}`);
...
export async function getKazusList(page = 1, limit = 20): Promise<{ kazuslar: any[]; total: number }> {
    try {
        const response = await apiRequest<any[]>(`/api/kazus?limit=${limit}`);
        return { kazuslar: response, total: response.length };
    } catch {
        return {
            kazuslar: [],
            total: 0,
...
export async function getRejectedList(page = 1, limit = 20): Promise<{ rejected: any[]; total: number }> {
    try {
        const response = await apiRequest<any[]>(`/api/rejected?limit=${limit}`);
        return { rejected: response, total: response.length };
    } catch {
        return {
            rejected: [],
            total: 0,
...
export async function getTemplatesList(page = 1, limit = 20): Promise<{ templates: any[]; total: number }> {
    try {
        const response = await apiRequest<any[]>(`/api/templates?limit=${limit}`);
        return { templates: response, total: response.length };
    } catch {
        return {
            templates: [],
            total: 0,

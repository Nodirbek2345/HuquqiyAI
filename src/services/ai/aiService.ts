    ...
    try {
        ...
        if (providerSettings.geminiEnabled !== false) {
            try {
                ...
                fallbackErrors.push(`Gemini: ${geminiError.message}`);
            }
        }
        ...
        if (providerSettings.groqEnabled !== false || providerSettings.openaiEnabled !== false) {
            try {
                ...
                fallbackErrors.push(`OpenAI/Groq: ${groqError.message}`);
            }
        }

        // 3. Local fallback (agar hammasi ishlamasa va localRules o'chirilmagan bo'lsa)
        if (providerSettings.localRulesEnabled === false) {
            const errorMsg = fallbackErrors.length > 0 
                ? `AI xizmatlarida xatolik yuz berdi: ${fallbackErrors.join(' | ')}. Mahalliy qoidalar tizimi o'chirilgan.`
                : "Barcha AI xizmatlari va Mahalliy Qoidalar (Zaxira) admin tomonidan o'chirilgan! Sozlamalarni tekshiring.";
        ...
        try {
            ...
            throw new Error(`Tizimda jiddiy xatolik: AI xizmatlari (${fallbackErrors.join(', ')}) va Mahalliy tahlil ham ishlamadi.`);
        }
    }
}

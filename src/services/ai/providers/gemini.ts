...
export class GeminiProvider implements AIProvider {
    ...
    async analyzeDocument(text: string, mode: AnalysisMode = 'quick'): Promise<AnalysisResult> {
        ...
        for (const apiKey of this.apiKeys) {
            try {
                logger.info(`Gemini: Kalit bilan urinilmoqda`);
                ...
                logger.info(`Gemini: Muvaffaqiyatli!`);
                ...
                logger.warn(`Gemini kalit ishlamadi: ${error.message}`);
                lastError = error;
    ...
    async createChat(context: string): Promise<ChatSession> {
        ...
        for (const apiKey of this.apiKeys) {
            try {
                ...
                logger.info(`Gemini Chat yaratildi`);
                break;
            } catch (e: any) {
                logger.warn(`Gemini Chat kalit ishlamadi: ${e.message}`);
                lastError = e;
                continue;
            }
        }

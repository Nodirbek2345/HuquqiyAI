...
const OptimizedTextEditor = React.forwardRef<TextEditorRef, {
  placeholder: string;
  isProcessing: boolean;
  fileName: string | null;
  onAnalyze: (text: string, fileName?: string) => void;
  theme: any;
}>(({ placeholder, isProcessing, fileName, onAnalyze, theme }, ref) => {
  const [localText, setLocalText] = useState("");

  React.useImperativeHandle(ref, () => ({
    setValue: (val: string) => setLocalText(val),
    getValue: () => localText
  }));
      ...
      <div className="flex flex-col sm:flex-row justify-between items-center sm:items-center px-6 sm:px-10 py-6 sm:py-8 bg-slate-50 border-t border-slate-100 gap-4 sm:gap-0">
        <div className="flex flex-col">
          ...
          <span className="text-lg font-black text-slate-900 uppercase">
            {localText.length.toLocaleString()} BELGI
          </span>
        </div>
        <button
          disabled={!localText.trim() || isProcessing}
          onClick={() => onAnalyze(localText, fileName || undefined)}
          className={`w-full sm:w-auto justify-center px-8 sm:px-10 py-4 rounded-2xl font-black text-sm transition-all flex items-center gap-3 shadow-xl active:scale-95 text-white uppercase tracking-widest
            ${localText.trim() ? `${theme.buttonBg}` : 'bg-slate-100 text-slate-300 cursor-not-allowed border-none shadow-none'}`}
        >
          {theme.buttonLabel}
...
const UploadSection: React.FC<UploadSectionProps> = ({ onAnalyze, onShowHistory, onBack, mode }) => {
  ...
  const processFile = async (file: File) => {
    ...
    try {
      ...
      editorRef.current?.setValue(`[Hujjat nomi: ${file.name}]\n\n${extractedText}`);
    } catch (error: any) {
      alert(error.message);
      setFileName(null);
    } finally {
          ...
          <div className="flex flex-col items-center">
            ...
            {fileName && !isProcessing && (
              <div className="mt-4 inline-flex items-center gap-3 px-6 py-2 rounded-full text-[11px] font-black bg-slate-50 border border-slate-100">
                ...
                <button onClick={(e) => { e.stopPropagation(); setFileName(null); editorRef.current?.setValue(''); }} className="hover:text-rose-600 ml-1">
                  <X className="w-4 h-4" />
                </button>
              </div>
            )}
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
            {[
              ...
              <button
                ...
                className={`group flex flex-col items-center gap-2 sm:gap-3 p-3 sm:p-6 bg-white rounded-2xl sm:rounded-3xl border-2 shadow-sm hover:shadow-xl hover:border-emerald-300 hover:-translate-y-1 transition-all duration-300 active:scale-95 border-slate-100`}
              >

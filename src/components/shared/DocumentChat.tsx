...
const MemoizedChatBubble = React.memo(({ msg }: { msg: Message }) => {
  return (
...
const DocumentChat: React.FC<DocumentChatProps> = ({ documentText }) => {
  ...
  const handleSendMessage = async () => {
    ...
    // Add user message, then empty AI placeholder
      ...
      for await (const chunk of responseStream) {
        if (chunk.text) {
          ...
          setMessages(prev => {
            ...
            const lastMsg = { ...newMessages[newMessages.length - 1] }; // yangi object qilib olyapmiz memo sezishi uchun
    ...
    <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden flex flex-col h-[500px] print-break-inside-avoid">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-gray-50/50">
        {messages.map((msg, idx) => (
          <MemoizedChatBubble key={idx} msg={msg} />
        ))}
        {isTyping && (
